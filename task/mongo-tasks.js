...
async function task_1_4(db) {
    const ordersCount = await db.collection('orders').aggregate([
        {
    ...
    const result = await db.collection('orders').aggregate([
        {
            $group: {
                _id: "$CustomerID",
                CustomerOrdersCount: { $sum: 1 },
                Total: {
                    $addToSet: ordersCount[0]["TotalOrdersCount"]
                }
            }
        },
        {
...
async function task_1_13(db) {
    const result = await db.collection('products').aggregate([
        {
            $facet: {
                TotalOfDiscontinuedProducts: [{ $match: { Discontinued: 1 } },],
...
async function task_1_15(db) {
    const result = await db.collection('orders').aggregate([
        ...
        {
            $facet: {
                January: [{ $match: { OrderMonth: { $eq: 1 } } }],
                February: [{ $match: { OrderMonth: { $eq: 2 } } }],
                March: [{ $match: { OrderMonth: { $eq: 3 } } }],
                April: [{ $match: { OrderMonth: { $eq: 4 } } }],
                May: [{ $match: { OrderMonth: { $eq: 5 } } }],
                June: [{ $match: { OrderMonth: { $eq: 6 } } }],
                July: [{ $match: { OrderMonth: { $eq: 7 } } }],
                August: [{ $match: { OrderMonth: { $eq: 8 } } }],
                September: [{ $match: { OrderMonth: { $eq: 9 } } }],
                October: [{ $match: { OrderMonth: { $eq: 10 } } }],
                November: [{ $match: { OrderMonth: { $eq: 11 } } }],
                December: [{ $match: { OrderMonth: { $eq: 12 } } }]
            },
        },
        {
            $project:
            {
                _id: 0,
                January: { $size: "$January" },
                February: { $size: "$February" },
                March: { $size: "$March" },
                April: { $size: "$April" },
                May: { $size: "$May" },
                June: { $size: "$June" },
                July: { $size: "$July" },
                August: { $size: "$August" },
                September: { $size: "$September" },
                October: { $size: "$October" },
                November: { $size: "$November" },
                December: { $size: "$December" },
            }
        }
    ]).toArray();
    return result[0];
    ...
    const result = await db.collection('categories').aggregate([
        ...
        },
        {
            $project: {
                _id: 0,
                CategoryName: 1,
                AvgPrice: { $round: [{ $avg: "$products.UnitPrice" }, 2] }
            }
        },
        {
...
async function task_1_18(db) {
    const result = await db.collection('orders').aggregate([
        {
            $addFields: {
                ODate: { $dateToString: { format: "%Y-%m-%d", date: { $toDate: "$OrderDate" } } },
                OrderYear: { $year: { $toDate: "$OrderDate" } }
            }
        },
        {
